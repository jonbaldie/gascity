...
func buildPreparedStartWithWorkDirResolver(
	...
	// fallback (session name). Skip this when pool_alias_conflict is set:
	// that stamp is the contested identity upstream scrubs so tmux emits
	// `env -u GC_ALIAS`.
	beadAlias := strings.TrimSpace(candidate.info.Alias)
	if beadAlias == "" && tp.EnvIdentityStamped && strings.TrimSpace(candidate.info.PoolAliasConflict) == "" {
