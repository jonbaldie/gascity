...
- **Session**: Start/stop/prompt/observe sessions regardless of
  ...
  [`internal/session/`](https://github.com/jonbaldie/gascity/tree/main/internal/session/);
  the runtime boundary is `runtime.Provider` in
  [`internal/runtime/`](https://github.com/jonbaldie/gascity/tree/main/internal/runtime/).
  Naming and startup hints live in
  [`internal/agent/`](https://github.com/jonbaldie/gascity/tree/main/internal/agent/).
...
- **Bead**: A single unit of work. Everything is a bead: tasks, mail,
  ...
  persistence substrate. See [`internal/beads/`](https://github.com/jonbaldie/gascity/tree/main/internal/beads/).
...
- **Config**: TOML parsing with progressive activation (Levels 0-8
  ...
  [`internal/config/`](https://github.com/jonbaldie/gascity/tree/main/internal/config/).
...
- **Event Bus**: Append-only pub/sub log of all system activity. Two
  ...
  [`internal/events/`](https://github.com/jonbaldie/gascity/tree/main/internal/events/).
...
- **Order**: A formula or shell script dispatch triggered by a
  ...
  [`internal/orders/`](https://github.com/jonbaldie/gascity/tree/main/internal/orders/).
...
- **Dispatch (Sling)**: The routing mechanism that composes: find/spawn
  ...
  [`cmd/gc/cmd_sling.go`](https://github.com/jonbaldie/gascity/blob/main/cmd/gc/cmd_sling.go).
...
- **Formula**: A `*.formula.toml` workflow definition discovered through
  ...
  validation lives in [`internal/convergence/formula.go`](https://github.com/jonbaldie/gascity/blob/main/internal/convergence/formula.go).
...
- **Trigger**: The trigger condition for an order. Types: `cooldown`
  ...
  invocation only). See
  [`internal/orders/triggers.go`](https://github.com/jonbaldie/gascity/blob/main/internal/orders/triggers.go).
...
- **Hook**: Provider-specific agent configuration files installed into
  ...
  [`internal/hooks/`](https://github.com/jonbaldie/gascity/tree/main/internal/hooks/).
...
- **Controller**: The long-running daemon that drives all SDK
  ...
  [`cmd/gc/controller.go`](https://github.com/jonbaldie/gascity/blob/main/cmd/gc/controller.go).
...
- **Overlay**: A directory tree copied into an agent's working
  ...
  See [`internal/overlay/`](https://github.com/jonbaldie/gascity/tree/main/internal/overlay/).
...
- **Pool**: Elastic scaling for an agent. The `PoolConfig` struct
  ...
  (`pool:<name>`). See [`cmd/gc/pool.go`](https://github.com/jonbaldie/gascity/blob/main/cmd/gc/pool.go).
...
- **Provider** (Session): Manages agent sessions. The `Provider`
  ...
  [`internal/runtime/runtime.go`](https://github.com/jonbaldie/gascity/blob/main/internal/runtime/runtime.go).
...
- **Rig**: An external project directory registered in the city. Each
  ...
  [`internal/config/config.go`](https://github.com/jonbaldie/gascity/blob/main/internal/config/config.go).
...
- **Pack**: A reusable agent configuration directory loaded from
  ...
  rig-scoped. See
  [`internal/config/pack.go`](https://github.com/jonbaldie/gascity/blob/main/internal/config/pack.go).
