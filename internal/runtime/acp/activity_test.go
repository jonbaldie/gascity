...
// TestStopTerminatesProviderProcessCrossProcess is the provider/session seam
// for drain-ack: a reconciler-owned Provider that did not Start the session
// must still terminate the OS process via the control socket.
func TestStopTerminatesProviderProcessCrossProcess(t *testing.T) {
	dir := filepath.Join(shortTempDir(t), "acp")
	owner := NewProviderWithDir(dir, Config{})
	name := testName()

	if err := owner.Start(context.Background(), name, runtime.Config{
		Command: fakeACPShellCommand(),
		WorkDir: t.TempDir(),
	}); err != nil {
		t.Fatalf("Start: %v", err)
	...
	if sc == nil || sc.cmd == nil || sc.cmd.Process == nil {
		t.Fatal("owner lost process handle after Start")
	}
	pid := sc.cmd.Process.Pid

	// Simulate the reconciler process: empty conns map, shared sidecar dir.
	reaper := NewProviderWithDir(dir, Config{})
	if !reaper.IsRunning(name) {
		t.Fatal("cross-process IsRunning = false before Stop")
	}
	if err := reaper.Stop(name); err != nil {
		t.Fatalf("cross-process Stop: %v", err)
	}
	if reaper.IsRunning(name) {
		t.Fatal("cross-process IsRunning = true after Stop")
	}
	if owner.IsRunning(name) {
		t.Fatal("owner IsRunning = true after cross-process Stop")
	}
	...
	for {
		...
		if time.Now().After(deadline) {
			t.Fatalf("provider OS process pid=%d still alive after cross-process Stop", pid)
		}
